...
def estimate_clean_ratio_baseline(
        ...
        try:
            for idx in range(len(doc)):
                try:
                    ...
                    cues = measure_page_visual_cues(image_jpeg, ratio_baseline=None)
                    ratio = float(cues.get("content_ratio") or 0.0)
                    if ratio > 0:
                        ratios.append(ratio)
                        page_count += 1
                except Exception:
    ...
    if not ratios:
        ...
        return {
            ...
            "ratio_tolerance": tol,
            "ratio_low": round(baseline * (1.0 - tol), 3),
            "ratio_high": round(baseline * (1.0 + tol), 3),
            "page_count": 0,
            "ratio_count": 0,
            "ratio_min": 0.0,
    ...
    lower_cluster_end = max(1, int(round(len(ratios_sorted) * 0.70)))
    lower_cluster = ratios_sorted[:lower_cluster_end]
    baseline = statistics.median(lower_cluster)
    mad = statistics.median([abs(x - baseline) for x in lower_cluster]) if lower_cluster else 0.0
    dynamic_tol = (2.5 * mad / baseline) if baseline > 0 else 0.0
    tol = max(0.05, min(0.08, dynamic_tol)) if dynamic_tol else 0.05
    return {
        "baseline_ratio": round(baseline, 3),
        "ratio_tolerance": round(tol, 4),
        "ratio_low": round(baseline * (1.0 - tol), 3),
        "ratio_high": round(baseline * (1.0 + tol), 3),
        "page_count": page_count,
        "ratio_count": len(ratios_sorted),
        "ratio_min": round(ratios_sorted[0], 3),
        "ratio_median": round(statistics.median(ratios_sorted), 3),
        "ratio_max": round(ratios_sorted[-1], 3),
        "ratio_p10": round(percentile(ratios_sorted, 0.10), 3),
        "ratio_p90": round(percentile(ratios_sorted, 0.90), 3),
    }


def measure_page_visual_cues(
    image_jpeg: bytes,
    ratio_baseline: Optional[float] = 2.24,
    ...
    try:
        ...
        bbox = mask.getbbox()
        baseline = float(ratio_baseline) if ratio_baseline else 0.0
        if baseline <= 0:
            baseline = 2.24
        tol = max(0.02, min(0.12, float(ratio_tolerance or 0.05)))
        ...
        cues: Dict[str, Any] = {
            ...
            "clean_ratio_baseline": round(baseline, 3),
            "clean_ratio_low": round(low, 3),
            "clean_ratio_high": round(high, 3),
        }
        if bbox:
            ...
            for x in range(crop_w):
                ...
                cols[x] = dark
            content_ratio = bw / bh
            content_ratio_error_pct = abs(content_ratio - baseline) / max(baseline, 1e-6)
            content_ratio_in_band = low <= content_ratio <= high
            prefix_dark = [0]
            for dark in cols:
                prefix_dark.append(prefix_dark[-1] + dark)
            suffix_col_max = [0] * (crop_w + 1)
            for i in range(crop_w - 1, -1, -1):
                suffix_col_max[i] = max(cols[i], suffix_col_max[i + 1])

            boundary_local_x = crop_w - 1
            boundary_strength = cols[boundary_local_x] / max(crop_h, 1) if crop_w else 0.0
            outside_dark = 0
            outside_area = 1
            outside_col_max = 0
            boundary_method = "bbox_right_edge"
            left_ratio = content_ratio
            boundary_error_pct = content_ratio_error_pct
            boundary_reliable = bool(content_ratio_in_band)
            boundary_candidates = 0

            if not content_ratio_in_band and content_ratio > high:
                search_start = max(2, int(crop_w * 0.55))
                search_end = max(search_start + 1, int(crop_w * 0.985))
                best_in_band: Optional[Tuple[float, int, float, float, int, int, float]] = None
                best_any: Optional[Tuple[float, int, float, float, int, int, float]] = None
                for x in range(search_start, search_end, 2):
                    left_bbox = crop.crop((0, 0, x + 1, crop_h)).getbbox()
                    if not left_bbox:
                        continue
                    left_bw = max(1, left_bbox[2] - left_bbox[0])
                    left_bh = max(1, left_bbox[3] - left_bbox[1])
                    if left_bh < int(crop_h * 0.65):
                        continue
                    candidate_left_ratio = left_bw / left_bh
                    candidate_error = abs(candidate_left_ratio - baseline) / max(baseline, 1e-6)
                    candidate_strength = cols[x] / max(crop_h, 1)
                    candidate_outside_dark = prefix_dark[-1] - prefix_dark[x + 1]
                    candidate_outside_area = max(1, (crop_w - x - 1) * crop_h)
                    candidate_outside_dark_ratio = candidate_outside_dark / candidate_outside_area
                    candidate_outside_col_max_ratio = suffix_col_max[x + 1] / max(crop_h, 1)
                    boundary_candidates += 1
                    ranking = (
                        candidate_error,
                        -x,
                        -candidate_strength,
                        -candidate_outside_dark_ratio,
                    )
                    payload = (
                        ranking[0],
                        x,
                        candidate_left_ratio,
                        candidate_strength,
                        candidate_outside_dark,
                        candidate_outside_area,
                        candidate_outside_col_max_ratio,
                    )
                    if low <= candidate_left_ratio <= high and candidate_strength >= 0.08:
                        if best_in_band is None or ranking < (
                            best_in_band[0],
                            -best_in_band[1],
                            -best_in_band[3],
                            -(best_in_band[4] / max(best_in_band[5], 1)),
                        ):
                            best_in_band = payload
                    if best_any is None or ranking < (
                        best_any[0],
                        -best_any[1],
                        -best_any[3],
                        -(best_any[4] / max(best_any[5], 1)),
                    ):
                        best_any = payload

                chosen = best_in_band or best_any
                if chosen:
                    _, boundary_local_x, left_ratio, boundary_strength, outside_dark, outside_area, outside_col_max_ratio = chosen
                    outside_col_max = int(round(outside_col_max_ratio * crop_h))
                    boundary_error_pct = abs(left_ratio - baseline) / max(baseline, 1e-6)
                    if best_in_band:
                        boundary_method = "ratio_guided_search"
                        boundary_reliable = True
                    else:
                        boundary_method = "ratio_guided_fallback"
                        boundary_reliable = False

            right_of_boundary_dark_ratio = outside_dark / max(outside_area, 1)
            right_of_boundary_colmax_ratio = outside_col_max / max(crop_h, 1)
            outside_structured_content = (
                right_of_boundary_dark_ratio >= 0.012 or right_of_boundary_colmax_ratio >= 0.10
            )
            cues.update(
                {
                    "content_width": bw,
                    ...
                    "content_ratio_in_clean_band": bool(content_ratio_in_band),
                    "content_ratio_error_pct": round(content_ratio_error_pct, 4),
                    ...
                    "ratio_guided_boundary_x": int(bbox[0] + boundary_local_x),
                    "ratio_guided_left_ratio": round(left_ratio, 3),
                    "ratio_guided_boundary_error_pct": round(boundary_error_pct, 4),
                    "ratio_guided_boundary_reliable": bool(boundary_reliable),
                    "ratio_guided_candidates": int(boundary_candidates),
                    "right_of_boundary_dark_ratio": round(right_of_boundary_dark_ratio, 4),
                    "right_of_boundary_colmax_ratio": round(right_of_boundary_colmax_ratio, 4),
                    "outside_structured_content": bool(outside_structured_content),
                }
            )

            cues.update(compute_right_strip_stats(img))
            cues["subtle_right_strip_suspect"] = subtle_right_strip_flag(cues, content_ratio_in_band)
        return cues
    except Exception:
        return {}
...
def classify_python_page(page_cues: Dict[str, Any], blurry_stats: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    content_ratio = float(page_cues.get("content_ratio") or 0.0)
    clean_low = float(page_cues.get("clean_ratio_low") or 0.0)
    clean_high = float(page_cues.get("clean_ratio_high") or 0.0)
    ...
    outside_dark = float(page_cues.get("right_of_boundary_dark_ratio") or 0.0)
    outside_col = float(page_cues.get("right_of_boundary_colmax_ratio") or 0.0)
    subtle = bool(page_cues.get("subtle_right_strip_suspect"))
    strip6_dark = float(page_cues.get("right_strip_dark_ratio_6") or 0.0)
    strip6_peak = int(page_cues.get("right_strip_peak_cols_6") or 0)
    strip6_max = float(page_cues.get("right_strip_maxcol_ratio_6") or 0.0)
    strip8_dark = float(page_cues.get("right_strip_dark_ratio_8") or 0.0)
    strip8_peak = int(page_cues.get("right_strip_peak_cols_8") or 0)
    strip8_max = float(page_cues.get("right_strip_maxcol_ratio_8") or 0.0)
    strip10_dark = float(page_cues.get("right_strip_dark_ratio_10") or 0.0)
    strip10_peak = int(page_cues.get("right_strip_peak_cols_10") or 0)
    strip10_max = float(page_cues.get("right_strip_maxcol_ratio_10") or 0.0)
    ...
    strong_subtle = bool(
        (strip6_dark >= 0.66 and strip6_peak >= 150 and strip6_max <= 0.93)
        or (strip8_dark >= 0.62 and strip8_peak >= 150 and strip8_max <= 0.93)
        or (strip10_dark >= 0.70 and strip10_peak >= 210 and strip10_max <= 0.93)
    )

    if subtle and strong_subtle:
        confidence = min(0.93, 0.82 + min(max(strip8_dark - 0.62, 0.0) * 0.30, 0.06))
        return {
            "decision": "overlap",
            "is_overlap": True,
            "is_blurry": False,
            "confidence": round(confidence, 3),
            "overlap_type": "ghost_superimposition",
            "signatures": [],
            "reason": "python_rule: extreme-right micro-strip shows an extra structured band hugging the far right edge",
        }

    if boundary_method == "ratio_guided_search" and boundary_reliable and outside_structured:
        extra_width = max(0.0, content_ratio - clean_high)
        confidence = min(0.97, 0.76 + min(extra_width / 0.55, 0.14) + min(outside_col * 0.12, 0.05) + min(outside_dark * 0.35, 0.05))
        return {
            "decision": "overlap",
            "is_overlap": True,
            "is_blurry": False,
            ...
            "signatures": [],
            "reason": "python_rule: ratio-guided first-page boundary is reliable and structured content remains to its right",
        }

    if boundary_method == "ratio_guided_fallback" and content_ratio > clean_high * 1.18 and (outside_dark >= 0.08 or outside_col >= 0.45):
        return {
            "decision": "overlap",
            "is_overlap": True,
            "is_blurry": False,
            ...
            "overlap_type": "clear_double_card",
            "signatures": [],
            "reason": "python_rule: page is materially wider than the clean ratio band and fallback boundary still leaves substantial right-side structure",
        }
    ...
    blurry_strong = bool(
        ...
        and blur_fill <= 0.16
        and not subtle
        and not outside_structured
        and content_ratio <= clean_high * 1.03
    )
    blurry_possible = bool(
        ...
        and blur_fill <= 0.20
        and not subtle
        and content_ratio <= clean_high * 1.05
    )
    ...
    if content_ratio > clean_high or subtle or blurry_possible:
        reason = "python_rule: page needs review"
        if content_ratio > clean_high:
            reason = "python_rule: page is wider than the learned clean ratio band, but evidence is not strong enough for hard overlap"
        elif subtle:
            reason = "python_rule: right-edge micro-strip looks suspicious, but not strong enough for hard overlap"
        elif blurry_possible:
    ...
    if in_band and boundary_method == "bbox_right_edge":
        return {
            "decision": "clean",
            "is_overlap": False,
            "is_blurry": False,
            ...
            "overlap_type": "none",
            "signatures": [],
            "reason": "python_rule: full visible card width is inside the clean ratio band and no extra right-edge structure is present",
        }

    if content_ratio < clean_low:
        return {
            "decision": "uncertain",
            "is_overlap": False,
            "is_blurry": False,
            ...
            "overlap_type": "none",
            "signatures": [],
            "reason": "python_rule: page is narrower than the learned clean band, so the heuristic cannot classify it confidently",
        }
    ...
    return {
        ...
        "overlap_type": "none",
        "signatures": [],
        "reason": "python_rule: local heuristic found no overlap or strong blurry evidence",
    }
...
class OverlapEngine:
    ...
    def scan_pdfs(
        ...
        if pdf_paths and (scope == "source" or not self.batch_ratio_info):
            try:
                self.log(f"Estimating clean single-card ratio baseline for {scope} pages...")
                self.batch_ratio_info = estimate_clean_ratio_baseline(pdf_paths)
                self.log(
                    "Ratio baseline: "
                ...
                self.log(f"Ratio baseline estimation failed, using default 2.24 +/- 5%: {exc}")
        elif self.batch_ratio_info:
            self.log(
                "Reusing ratio baseline: "
                f"baseline={self.batch_ratio_info.get('baseline_ratio')} "
                f"band={self.batch_ratio_info.get('ratio_low')}..{self.batch_ratio_info.get('ratio_high')}"
            )
        ...
        for pdf_path in pdf_paths:
            ...
            for idx in range(len(doc)):
                ...
                try:
                    ...
                    if page_cues.get("content_ratio_in_clean_band"):
                        try:
                            _highres_right_strip_jpeg, highres_strip_cues = render_highres_right_strip_jpeg(doc[idx], dpi=360)
                            page_cues.update(highres_strip_cues)
                            page_cues["subtle_right_strip_suspect"] = subtle_right_strip_flag(
                                page_cues, bool(page_cues.get("content_ratio_in_clean_band"))
                            )
                    ...
                    rec = {
                        ...
                        "scope": scope,
                        "model": "PY",
                        "resolved_model": "python_heuristic",
                ...
                file_records.append(rec)
                if rec.get("decision") == "uncertain":
                    self.log("Page result (python uncertain): " + summarize_page_result(rec))
                else:
                    self.log("Page result (python): " + summarize_page_result(rec))
                if on_page_result:
                    try:
                        on_page_result(rec, pdf_path, doc)
                    except Exception as cb_exc:
...
def export_single_tagged_page_from_doc(
    doc: fitz.Document, src_path: Path, page: int, prefix: str, logger, output_dir: Optional[Path] = None
) -> bool:
    out_root = output_dir or src_path.parent
    out_root.mkdir(parents=True, exist_ok=True)
    out = out_root / f"{prefix}_{src_path.stem}_P{page}.pdf"
    try:
        one = fitz.open()
...
def export_single_overlap_page_from_doc(
    ...
    return export_single_tagged_page_from_doc(doc, src_path, page, "O", logger, output_dir=output_dir)


def export_single_blurry_page_from_doc(
    doc: fitz.Document, src_path: Path, page: int, logger, output_dir: Optional[Path] = None
) -> bool:
    return export_single_tagged_page_from_doc(doc, src_path, page, "B", logger, output_dir=output_dir)
...
def export_overlap_pages(records: List[Dict[str, Any]], logger, output_dir: Optional[Path] = None) -> int:
    ...
    for file_path, pages in by_file.items():
        ...
        for page in sorted(set(pages)):
            ...
            out_root.mkdir(parents=True, exist_ok=True)
            out = out_root / f"O_{src.stem}_P{page}.pdf"
            try:
                one = fitz.open()
                one.insert_pdf(doc, from_page=page - 1, to_page=page - 1)
                one.save(str(out))
...
def export_blurry_pages(records: List[Dict[str, Any]], logger, output_dir: Optional[Path] = None) -> int:
    ...
    for file_path, pages in by_file.items():
        ...
        try:
            ...
            continue
        for page in sorted(set(pages)):
            if export_single_blurry_page_from_doc(doc, src, page, logger, output_dir=output_dir):
                created += 1
        doc.close()
    return created
...
class App(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        self.llm_mode_var = tk.BooleanVar(value=True)
        self.py_mode_var = tk.BooleanVar(value=False)
        ...
        ttk.Label(source_box, text="Mode").grid(row=0, column=0, sticky="w", padx=8, pady=(8, 4))
        mode_row = ttk.Frame(source_box)
        mode_row.grid(row=0, column=1, sticky="w", padx=8, pady=(8, 4))
        ttk.Checkbutton(mode_row, text="LLM", variable=self.llm_mode_var, command=lambda: self.select_detector_mode("llm")).pack(side=tk.LEFT)
        ttk.Checkbutton(mode_row, text="PY", variable=self.py_mode_var, command=lambda: self.select_detector_mode("py")).pack(side=tk.LEFT, padx=(10, 0))

        ttk.Label(source_box, text="Model").grid(row=1, column=0, sticky="w", padx=8, pady=4)
        self.model_combo = ttk.Combobox(source_box, textvariable=self.model_name_var, state="readonly")
        self.model_combo.grid(row=1, column=1, sticky="w", padx=8, pady=4)
        self.model_combo.bind("<<ComboboxSelected>>", self.on_select_model)

        ttk.Label(source_box, text="Scan Directory").grid(row=2, column=0, sticky="w", padx=8, pady=4)
        self.source_dir_entry = ttk.Entry(source_box, textvariable=self.source_dir_var)
        self.source_dir_entry.grid(row=2, column=1, sticky="ew", padx=8, pady=4)
        ttk.Button(source_box, text="Browse", command=self.pick_source_dir).grid(row=2, column=2, sticky="ew", padx=(0, 8), pady=4)

        ttk.Label(source_box, text="Output Directory").grid(row=3, column=0, sticky="w", padx=8, pady=(4, 8))
        self.output_dir_entry = ttk.Entry(source_box, textvariable=self.output_dir_var)
        self.output_dir_entry.grid(row=3, column=1, sticky="ew", padx=8, pady=(4, 8))
        ttk.Button(source_box, text="Browse", command=self.pick_output_dir).grid(row=3, column=2, sticky="ew", padx=(0, 8), pady=(4, 8))
        ...
        run_box.grid(row=2, column=0, columnspan=2, sticky="ew", pady=(10, 0))
        ...
        log_box.grid(row=3, column=0, columnspan=2, sticky="nsew", pady=(10, 0))
    ...
    def _load_defaults(self) -> None:
        ...
        if names:
            self.model_combo.configure(width=max(len(name) for name in names) + 2)
        if names:
            self.model_combo.current(0)
            self.on_select_model()
        self.select_detector_mode("llm")
    ...
    def select_detector_mode(self, mode: str) -> None:
        mode = "py" if mode == "py" else "llm"
        self.llm_mode_var.set(mode == "llm")
        self.py_mode_var.set(mode == "py")
        if hasattr(self, "model_combo"):
            self.model_combo.configure(state="readonly" if mode == "llm" else "disabled")

    def selected_detector_mode(self) -> str:
        return "py" if self.py_mode_var.get() else "llm"
    ...
    def run_pipeline(self) -> None:
        ...
        detector_mode = self.selected_detector_mode()
        profile = self.current_profile() if detector_mode == "llm" else None
        if detector_mode == "llm" and not profile:
            messagebox.showerror("Error", "No model selected.")
            return
        ...
        mode_label = profile.name if profile else "PY"
        self._set_status(f"Starting {mode_label} scan...")
        ...
        def worker() -> None:
            ...
            try:
                ...
                self.log(f"Detection mode: {detector_mode.upper()}")
                self.log(
                ...
                if not src_pdfs:
                    ...
                    return
                self._set_status(f"Scanning {len(src_pdfs)} source PDFs with {mode_label}...")
                self.log(f"Source PDFs: {len(src_pdfs)}")

                if detector_mode == "llm":
                    client = OpenAICompatibleClient(profile)
                    verifier_client: Optional[OpenAICompatibleClient] = None
                    if profile.name in {"GPT-5.4", "GPT-5.3-Codex"}:
                        kimi_profile = next((m for m in self.models if m.name == "Kimi-K2.5"), None)
                        if kimi_profile:
                            verifier_client = OpenAICompatibleClient(kimi_profile)
                            self.log(
                                f"Secondary verifier enabled: primary={profile.name}, "
                                f"verifier={kimi_profile.name}"
                            )
                        else:
                            self.log("Secondary verifier unavailable: Kimi-K2.5 profile not found.")
                    ...
                    self.log("Using local Python heuristic detector. No API requests will be sent.")
                ...
                def on_source_page_result(rec: Dict[str, Any], pdf_path: Path, doc: fitz.Document) -> None:
                    ...
                    if live_output and self.act_identify_var.get() and rec.get("is_overlap"):
                        if export_single_overlap_page_from_doc(doc, pdf_path, int(rec["page"]), self.log, output_dir=output_dir):
                            live_o_count += 1

                    if live_output and self.act_blurry_var.get() and rec.get("is_blurry"):
                        if export_single_blurry_page_from_doc(doc, pdf_path, int(rec["page"]), self.log, output_dir=output_dir):
                            live_b_count += 1
                ...
                if total_source and overlap_count / total_source >= 0.5:
                    self.log(
                        "Warning: overlap rate is unusually high for this run. "
                        "Review a few clean-looking pages and consider switching models."
                    )
                ...
                if self.act_identify_var.get():
                    if live_output:
                        ...
                        cnt = export_overlap_pages(source_records, self.log, output_dir=output_dir)
                ...
                if self.act_blurry_var.get():
                    if live_output:
                        ...
                        cnt = export_blurry_pages(source_records, self.log, output_dir=output_dir)
